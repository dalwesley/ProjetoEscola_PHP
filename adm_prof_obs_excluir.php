...
<?php
include "conecta.php";
$sql = mysql_query("SELECT * from professor_obs where prof_obs_id=$id_select2");
...
$prof_obs_data	 					= date('d-m-y');


echo "<div id=div1>
<form id='excluir' name='excluir' method='post' action='adm_prof_obs_excluir_motor.php' enctype='multipart/form-data'>
...
<table border='1' width='100%' cellspacing='0' cellpadding='3'  bordercolor='#F0F0F0' height='0' style='border-collapse: collapse'> 
	<tr>
		...
		<td>
			<center>Excluindo <B><I>observação</B></I> para <i><strong>$prof_obs_prof_nome - $prof_ra</center></i></strong>
		</td>
		<td height='1' width='1'>
...
<tr>
	...
	<td align='center'><input type='checkbox' name='cod[]' value=$linha[prof_obs_id] required/>
	</td>
	<td><pre>
Def. Visual?		<B>$prof_obs_visual - $prof_obs_visual_qual</B>
Def. Auditiva?		<B>$prof_obs_auditiva - $prof_obs_auditiva_qual</B>
Def. Fisica?		<B>$prof_obs_fisica - $prof_obs_fisica_qual</B>
Def. Movimento?		<B>$prof_obs_movimento - $prof_obs_movimento_qual</B>
Def. Comportamento?	<B>$prof_obs_comportamento - $prof_obs_comportamento_qual</B>
Usa Fraldas?		<B>$prof_obs_fraldas</B>
Cadeirante?		<B>$prof_obs_cadeira_rodas</B>

Observações?	<B>$prof_obs_mencionar_prob_import</B>
</td>
</tr>
</table>";
}
...
<table border='1' width='100%' cellspacing='0' cellpadding='3'  bordercolor='#FFFFFF' height='0' style='border-collapse: collapse'> 
	<tr>
		<td align=center bgcolor=#FFFFFF><font color='#FF0000'>Tem certeza que deseja exluir a <b>observação</b> para <b>$prof_nome</b>?</b></font><br></td>	
	</tr>	
	<tr>
		<td align='center'>
